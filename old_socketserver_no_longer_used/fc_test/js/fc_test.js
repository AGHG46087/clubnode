...
jQuery( function ( $ ) {
    ...
    sensibleJsonToHtml: function ( obj ) {
      ...
      var visit = function ( obj, indentLevel ) {
        ...
        if ( $.isArray( obj ) ) {
          ...
          return '<span class="json-punctuation">[</span>' + separator + items.join( ',' + separator ) + separator + indentLevel + '<span class="json-punctuation">]</span>';

        } else {
          // Dictionary object
          for ( var k in obj ) {
            items.push( nextIndent + '<span class="json-key">' + Utils.escape( JSON.stringify( k ) ) + '</span>' + '<span class="json-punctuation">:</span> ' + visit( obj[k], nextIndent ) );
          }
          return '<span class="json-punctuation">{</span>' + separator + items.join( ',' + separator ) + separator + indentLevel + '<span class="json-punctuation">}</span>';
        }
      }
  ...
  Device.prototype = {
    ...
    isEqual: function ( json ) {
      return this.json.type == json.type && this.json.serial == json.serial && this.json.timestamp == json.timestamp;
    },
    ...
    makeGradientColor: function(frequency1, frequency2, frequency3,
                                phase1, phase2, phase3,
                                center, width, len) {

      var device = this;
      var blackness = device.singleColorFrame( 0, 0, 0 );

      device.writePixels( blackness );
      device.writePixels( blackness );
    ...
    makeAColor: function(frequency1, frequency2, frequency3, phase1, phase2, phase3, width, center, len) {
    ...
    initTypeFadecandy: function () {
      ...
      this.view.find( ".list-group-item-heading" ).text( "Found FC LED Controller" ).after( '\
                    <p> \
                    ...
                    <div class="btn-group"> \
                        ...
                        <div class="btn-group"> \
                            ...
                            <ul class="dropdown-menu" role="menu"> \
                                ...
                                <li><a class="action-cycle" href="#">Cycle RGB</a></li> \
                                <li><a class="action-half" href="#">50% brightness</a></li> \
                                <li><a class="action-full" href="#">Full Rainbow</a></li> \
                            </ul> \
                        </div> \
                    </div> \
                ' );
      ...
      this.view.find( ".action-identify" ).tooltip( {
          placement: "bottom", title: "Identify this device by flashing its built-in status LED", container: "body",
        } ).click( function ( evt ) {
          var m = device.modalAction( "Identifying Fadecandy Device", ' \
                        <p> \
...
//        device.fadeInSingleFrame( device.singleColorFrame( 255, 255, 255 ) );
        ...
        interval = setInterval(function(){
          ...
          var colors = device.makeAColor(.3,.3,.3,0,2,4, 128, 127, intervalCount);
          device.writePixels( device.singleColorFrame( colors[0], colors[1], colors[2] ));
        }, 200);
      } );
      this.view.find( ".action-cycle" ).click( function ( evt ) {
        setTimeout(function() {
          device.fadeInSingleFrame( device.singleColorFrame( 128, 0, 0 ));
          setTimeout(function() {
            device.fadeInSingleFrame( device.singleColorFrame( 0, 128, 0 ));
            setTimeout(function(){
              device.fadeInSingleFrame( device.singleColorFrame( 0, 0, 128 ));
              setTimeout(function(){
                device.fadeInSingleFrame( device.singleColorFrame( 0, 0, 0 ));
              }, 1500);
            },1500);
    ...
    initTypeOther: function () {
      ...
      this.view.find( ".list-group-item-heading" ).text( "Other Device" ).after( '<p>Properties:</p>', '<pre></pre>' );
  ...
  var ConnectionManager = {
    ...
    connect: function () {
       ...
       */
      var text = document.getElementById( 'ipaddr' ).value;
      text = "ws://" + text + ":7890/";
